(function () {
    ...
    angular
        ...
        .directive('sentinelTrackingFilterbar', SentinelTrackingFilterbar);

    function SentinelTrackingFilterbar() {
        var directive = {
            restrict: 'A',
            controller: ThisDirectiveController,
            controllerAs: 'sentinelTrackingFilterbar',
            templateUrl: 'ui-sentinel-devices.sentinelTracking/sentinel-tracking-filterbar-directive.html'
        };
        return directive;
    }

    ThisDirectiveController.$inject = ['$scope', '$state', 'SentinelTrackingReportsService', 'SentinelTrackingFilterService', 'DatetimeValidatorService'];
    function ThisDirectiveController($scope, $state, SentinelTrackingReportsService, SentinelTrackingFilterService, DatetimeValidatorService) {
        var vm = {
            reportsService: SentinelTrackingReportsService,
            filter: SentinelTrackingFilterService,
            error: null,
            fromDate: {
                label: null,
                date: null,
        ...
        function activate() {
            $('#sentinelTrackingFromDate').datepicker();
            $('#sentinelTrackingFromTime').timepicker({ 'timeFormat': DatetimeValidatorService.localTimeFormat()});
            $('#sentinelTrackingToDate').datepicker();
            $('#sentinelTrackingToTime').timepicker({ 'timeFormat': DatetimeValidatorService.localTimeFormat()});
        ...
        function navToDeviceMap() {
            $state.go('device.sentinelmap', { deviceTagId: $state.params.deviceTagId });
        }
        ...
        function navToDeviceSummary() {
            $state.go('device.sentinelsummary', { deviceTagId: $state.params.deviceTagId });
        }
