(function() {
    ...
    ForgotController.$inject = ['$scope', '$state','LoginsApiService', 'FeedbackService'];
    function ForgotController($scope, $state, LoginsApiService, FeedbackService) {
        var vm = {
            email: {
                value: null,
                isPristine: true,
        ...
        function submitEmail() {
            $('#btn-submitEmail').blur();
            validateEmail();

            if (vm.email.hasError())
                return;

            vm.feedback.clear();
            var promise = LoginsApiService.forgotPassword(vm.email.value).$promise;
            promise.then(
                function (result) {
                    vm.emailSent = true;
                ...
                function (error) {
                    ...
                    vm.feedback.addError('Unable to send email.  Please try again later.');
                }
            );
        }
        ...
        function submitToken() {
            ...
            if (vm.password.hasError() || vm.token.hasError() || vm.confirmation.hasError())
                return;

            var promise = LoginsApiService.confirmResetToken(vm.email.value, vm.token.value.trim(), vm.password.value.trim()).$promise;
            promise.then(
                function (result) {
