(function () {
    ...
    LatestDeviceTrackingMapController.$inject = ['$rootScope', '$scope','$filter', '$state', 'SentinelUiSession','localStorageService', 'LatestDeviceTrackingReportsService', 'LatestDeviceTrackingFilterService', 'MapsConstants', 'UomSecondsConverter'];
    function LatestDeviceTrackingMapController($rootScope, $scope,$filter, $state, SentinelUiSession, localStorageService, LatestDeviceTrackingReportsService, LatestDeviceTrackingFilterService, MapsConstants, UomSecondsConverter) {
        var googleMapDivId = 'latestDeviceTrackingMap';
        ...
        function goToDeviceMap() {
            if (vm.reportsService.selected) {
                $state.go('device.map',{ deviceTagId: vm.reportsService.selected.deviceTagId, referrer: 'devices.map'  });
            }
        }

        function goToSentinelMap() {
            if (vm.reportsService.selected) {
                $state.go('device.sentinelmap',{ deviceTagId: vm.reportsService.selected.deviceTagId, referrer: 'devices.map'  });
            }
        }
        ...
        function goToDeviceReports() {
            ...
            if (vm.reportsService.selected && vm.reportsService.selected.type == "Sentry") {
                ...
                $state.go('sentry-reports.by-device', { imei: vm.reportsService.selected.deviceTagId, view: 'prior', to: to, from: from});
                ...
                from = moment(vm.reportsService.selected.timeOfReceipt).subtract(1, 'days').toISOString();
                $state.go('sightings.of-mac', { mac: vm.reportsService.selected.deviceTagId, to: vm.reportsService.selected.timeOfReceipt, from: from});
            }
        }

        function gotoSightingsForReport() {
            if (vm.reportsService.selected && vm.reportsService.selected.type == "Sentry") {
                $state.go('sightings.for-report', { reportId: vm.reportsService.selected.reportGuid, reportsList: vm.reportsService.reports });
            }
        }

        function goToDeviceAdmin() {
            if (vm.reportsService.selected &&  vm.reportsService.selected.type === "Sentry") {
               $state.go('sentry-configs.by-device', { assignmentAccountId: vm.reportsService.selected.accountId, imei: vm.reportsService.selected.deviceTagId });
            }
        }
